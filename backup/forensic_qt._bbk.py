...
# ══════════════════════════════════════════════════════════════
#  FORENSIC IMAGE FILESYSTEM  (pytsk3 + pyewf / ewfmount)
#  Auto-selects backend:
#    Windows / macOS  ->  pyewf   (pip install pyewf)
#    Linux fallback   ->  ewfmount FUSE (apt install ewf-tools)
#    DD / RAW         ->  pytsk3 direct (no extra library)
# ══════════════════════════════════════════════════════════════

def _ewf_available():
    """Probe for E01 backend. Returns ('pyewf', mod), ('ewfmount', None), or (None,None)."""
    try:
        import pyewf
        return ('pyewf', pyewf)
    except ImportError:
...
class ForensicImageFS:
    ...
    Supports: E01/EWF (via pyewf or ewfmount), DD, RAW, ISO, VMDK, VHD.
    Use ForensicImageFS.get(path) — instances are cached per path.
    """
    _cache      = {}   # path -> ForensicImageFS
    _mount_dirs = {}   # path -> tmp FUSE mount dir

    def __init__(self, image_path):
        self.image_path  = image_path
        self.fs          = None    # pytsk3.FS_Info (active partition)
        self.img         = None    # pytsk3.Img_Info
        self.partitions  = []      # list of {desc, offset, size, addr}
        self.active_part = 0
        self.error       = ""
        self._mount_dir  = None    # FUSE mount point (ewfmount only)
        self._ewf_handle = None    # open pyewf.handle (must stay alive)
        self._open()
    ...
    def _open_ewf(self, pytsk3):
        method, pyewf_mod = _ewf_available()

        if method == 'pyewf':
            # Windows, macOS, Linux — native pyewf binding
            try:
                filenames = pyewf_mod.glob(self.image_path)
                if not filenames:
                    filenames = [self.image_path]
                handle = pyewf_mod.handle()
                handle.open(filenames)
                self._ewf_handle = handle
                # pytsk3 accepts a pyewf handle directly as an Img_Info source
                self.img = pytsk3.Img_Info(handle)
                ...
                if platform.system() == 'Windows':
                    ...
                    return
                # On Linux, fall through to ewfmount

        if method == 'ewfmount' or (
                method != 'pyewf' and platform.system() != 'Windows'):
            if shutil.which('ewfmount'):
                self._open_ewf_fuse(pytsk3)
                return

        self.error = (
            "No EWF/E01 library found. "
            "Windows/macOS: pip install pyewf   "
            "Linux: sudo apt install ewf-tools  OR  pip install pyewf"
        )

    def _open_ewf_fuse(self, pytsk3):
        """Linux-only: mount E01 via ewfmount FUSE, then open exposed raw file."""
        ...
        if ret != 0:
            self.error = (
                "ewfmount failed (exit %d). "
                "Try: sudo apt install ewf-tools  OR  pip install pyewf" % ret)
            return
        ...
        if not ewf1:
            self.error = "ewfmount succeeded but produced no output in %s" % mnt
            return
        self._open_raw(pytsk3, ewf1)

    def _open_raw(self, pytsk3, raw_path):
        """Open a raw/DD image (or ewfmount-exposed file) directly."""
        try:
    ...
    def _finish_open(self, pytsk3):
        """Enumerate partitions then open the first readable filesystem."""
        # Try MBR / GPT partition table
        try:
            vol = pytsk3.Volume_Info(self.img)
            for part in vol:
                desc = part.desc.decode(errors='replace').strip()
                if any(s in desc for s in
                       ('Unallocated', 'Safety', 'Meta', 'Table', 'Primary Table')):
                    continue
            ...
            pass    # no partition table — try whole image below

        # No partition table, or no partition yielded a FS
        try:
            self.fs = pytsk3.FS_Info(self.img)
            if not self.partitions:
                try:   sz = os.path.getsize(self.image_path)
                except: sz = 0
                self.partitions = [{'desc': 'Whole Image',
                                     'offset': 0, 'size': sz, 'addr': 0}]
        except Exception as e:
            self.error = (
                "No recognisable filesystem found. %s  "
                "Supported: NTFS, FAT12/16/32, exFAT, ext2/3/4, ISO9660, HFS+" % e)
    ...
    def switch_partition(self, idx):
        ...
        try:
            import pytsk3
            self.fs = pytsk3.FS_Info(self.img,
                                     offset=self.partitions[idx]['offset'])
            self.active_part = idx
            return True
        except Exception as e:
            self.error = str(e)
            ...
            for e in d:
                try:
                    ...
                    entries.append({
                        ...
                        'type':   'Directory' if is_dir
                                  else detect_type_by_name(name),
                    })
                except Exception:
                    pass
        except Exception as e:
            entries.append({'name': '[Error: %s]' % e, 'is_dir': False,
                            'size': 0, 'mtime': 0, 'atime': 0,
                            'ctime': 0, 'inode': 0, 'type': 'Error'})
    ...
    def fs_type_str(self):
        if not self.fs:
            return 'Unknown'
        return {
            2: 'FAT12', 4: 'FAT16', 8: 'FAT32',
            0x0b: 'exFAT', 0x80: 'ext2', 0x81: 'ext3',
            0x82: 'ext4',  0x03: 'NTFS', 0x0c: 'ISO9660',
            0x10: 'HFS+',  0x12: 'YAFFS2',
        }.get(self.fs.info.ftype, 'FS type %d' % self.fs.info.ftype)

    def ewf_backend(self):
        method, _ = _ewf_available()
        return method or 'none'


def detect_type_by_name(name):
    """Detect file type from extension alone (no disk access needed)."""
    return {
