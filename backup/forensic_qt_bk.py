...
# ══════════════════════════════════════════════════════════════
#  FORENSIC IMAGE FILESYSTEM  (pytsk3 + ewfmount)
# ══════════════════════════════════════════════════════════════

class ForensicImageFS:
    """
    Wraps pytsk3 to provide filesystem access for DD/raw images directly,
    and E01/EWF images via ewfmount (FUSE).
    Keeps a cache of open FS handles per image path.
    """
    _cache = {}          # path -> ForensicImageFS instance
    _mount_dirs = {}     # path -> tmp mount dir (for E01)

    def __init__(self, image_path):
        self.image_path   = image_path
        self.fs           = None       # pytsk3.FS_Info
        self.img          = None       # pytsk3.Img_Info
        self.partitions   = []         # list of (desc, offset_bytes, fs_type_hint)
        self.active_part  = 0
        self.error        = ""
    ...
    def _open(self):
        ...
        actual_path = self.image_path

        # ── E01/EWF: mount via ewfmount ─────────────────────────────
        if ext in ('.e01', '.ewf', '.ex01'):
            mnt = tempfile.mkdtemp(prefix='fpro_ewf_')
            self._mount_dir = mnt
            ForensicImageFS._mount_dirs[self.image_path] = mnt
            ret = os.system(f'ewfmount "{self.image_path}" "{mnt}" 2>/dev/null')
            if ret != 0:
                self.error = (
                    "ewfmount failed (exit %d). "
                    "Ensure ewf-tools is installed and the image is not corrupted." % ret)
                return
            # ewfmount exposes ewf1 inside the mount dir
            ewf1 = os.path.join(mnt, 'ewf1')
            if not os.path.exists(ewf1):
                candidates = os.listdir(mnt)
                if candidates:
                    ewf1 = os.path.join(mnt, candidates[0])
                else:
                    self.error = "ewfmount succeeded but no file found in %s" % mnt
                    return
            actual_path = ewf1

        # ── Open with pytsk3 ────────────────────────────────────────
        try:
            self.img = pytsk3.Img_Info(actual_path)
        except Exception as e:
            self.error = "pytsk3.Img_Info: %s" % e
            return

        # Try Volume_Info first (handles MBR/GPT partition tables)
        try:
            vol = pytsk3.Volume_Info(self.img)
            for part in vol:
                desc = part.desc.decode(errors='replace').strip()
                offset_bytes = part.start * vol.info.block_size
                # Skip unallocated / metadata partitions
                if 'Unallocated' in desc or 'Safety' in desc:
                    continue
            ...
            # Open FS on first usable partition
            for p in self.partitions:
                try:
                    self.fs = pytsk3.FS_Info(self.img, offset=p['offset'])
                    self.active_part = self.partitions.index(p)
                    return
                except Exception:
                    continue
        except Exception:
        ...
        # No partition table — try direct FS at offset 0
        try:
            self.fs = pytsk3.FS_Info(self.img)
            self.partitions = [{'desc': 'Whole Image', 'offset': 0,
                                 'size': os.path.getsize(actual_path), 'addr': 0}]
        except Exception as e:
            self.error = "Cannot detect filesystem: %s" % e

    def switch_partition(self, idx):
        """Switch active FS to partition index idx."""
    ...
    def list_dir(self, inode=None, path=None):
        """
        Return list of dicts for directory contents.
        Each dict: name, is_dir, size, mtime, atime, ctime, inode, type_str
        """
        import pytsk3
        if not self.fs:
            return []
    ...
    def read_file(self, inode, max_bytes=4*1024*1024):
        """Read file content by inode number."""
        if not self.fs:
            return b""
        try:
            ...
            return b"[Read error: %s]" % str(e).encode()

    def cleanup(self):
        """Unmount ewf if mounted."""
...
def detect_type_by_name(name):
    ...
    return {
        ...
        ".7z":"7-Zip Archive",".tar":"TAR Archive",".gz":"GZip",
