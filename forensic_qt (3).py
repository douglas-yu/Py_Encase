...
class BookmarkTab(QWidget):
    ...
    def add_bookmark(self, artifact_name: str, row_data: dict, tag: str):
        try:
            ...
            # ── Defer all post-insert UI work to next event loop tick ──
            # This prevents any signal fired by setCurrentRow / setCurrentIndex
            # from touching the table while we are still inside add_bookmark.
            ...
            def _post_insert():
                try:
                    self._update_count()
                    # Switch sidebar without retriggering table selection
                    self.cat_list.blockSignals(True)
                    self.cat_list.setCurrentRow(_cat_idx)
                    self.cat_list.blockSignals(False)
                    # Stack switch is safe now — table is fully populated
                    self._stack.setCurrentIndex(_cat_idx)
                    self._cur_idx = _cat_idx
                    # Scroll to new row by matching timestamp
                    for r in range(_tbl.rowCount()):
                        ti = _tbl.item(r, 2)
                        if ti and ti.text() == _ts:
                            _tbl.scrollToItem(
                                _tbl.item(r, 0),
                                QAbstractItemView.ScrollHint.EnsureVisible)
                            break
                except Exception as ex:
                    print(f"[BookmarkTab._post_insert] {ex}")
